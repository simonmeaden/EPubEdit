...
void
EBookMetadata::parse(QDomNodeList metadataNodeList)
{
...
bool
EBookMetadata::write(QXmlStreamWriter* xmlWriter)
{

  xmlWriter->writeStartElement("metadata");

  if (!m_orderedTitles.isEmpty()) {
    QList<int> keys = m_orderedTitles.keys();
    // This may contain some extra titles added by dcterms:title
    QStringList namedTitles = m_titlesByName.keys();
    foreach (int key, keys) {
      QString title = writeTitleMetadata(xmlWriter, key);
      namedTitles.removeAll(title);
    }
    // These were probably added from dcterms:title metas.
    foreach (QString title, namedTitles) {
      Title sharedTitle = m_titlesByName.value(title);
      writeTitle(xmlWriter, sharedTitle);
    }
  }
  ...
  if (!m_creatorsByName.isEmpty()) {
    ...
    foreach (QString key, keys) {
      QString name = writeCreatorsMetadata(xmlWriter, key);
    ...
    foreach (QString creator, ids) {
      ...
      writeCreator(xmlWriter, shared_creator);
    }
  }

  if (!m_contributorsByName.isEmpty()) {
    QStringList keys = m_contributorsByName.keys();
    foreach (QString key, keys) {
      writeContributorMetadata(xmlWriter, key);
    }
  }

  if (!m_description.isNull()) {
    writeDescriptionMetadata(xmlWriter);
  }
  ...
  if (!m_languages.isEmpty()) {
    ...
    if (keys.size() == 1) {
      writeLanguageMetadata(xmlWriter, m_languages.keys().at(0), true);
    } else {
      bool first = true;
      foreach (QString key, keys) {
        writeLanguageMetadata(xmlWriter, key, first);
        first = false;
      }
    }
  }
  ...
  if (!m_subjects.isEmpty()) {
    ...
    foreach (QString key, keys) {
      writeSubjectMetadata(xmlWriter, key);
    }
  }
  ...
  if (!m_identifiers.isEmpty()) {
    ...
    foreach (EBookIdentifierScheme::IdentifierScheme key, keys) {
      writeIdentifierMetadata(xmlWriter, key);
    }
  }

  if (!m_date.isNull()) {
    xmlWriter->writeStartElement("date");
    xmlWriter->writeCharacters(m_date.toString(Qt::ISODate));
    xmlWriter->writeEndElement();
  }

  if (!m_source.isNull()) {
    writeSourceMetadata(xmlWriter);
  }

  if (!m_publisher.isNull()) {
    writePublisherMetadata(xmlWriter);
  }

  if (!m_format.isNull()) {
    writeFormatMetadata(xmlWriter);
  }

  if (!m_relation.isNull()) {
    writeRelationMetadata(xmlWriter);
  }

  if (!m_.isNull()) {
    writeRightsMetadata(xmlWriter);
  }

  if (!m_coverage.isNull()) {
    writeCoverageMetadata(xmlWriter);
  }
...
{
  m_orderedTitles = ordered_titles;
}
                                   ...
                                   QDomNamedNodeMap& node_map)
{
  if (tag_name == "title") {
    ...
    m_date = QDateTime::fromString(metadata_element.text(), Qt::ISODate);
    if (!id_node.isNull()) {
                                              ...
                                              QDomNamedNodeMap node_map)
{
  ...
  if (!node.isNull()) {
    ...
    if (property == "role") {
      ...
      shared_creator->alt_RepList.append(alt_rep);
      ...
      if (m_creatorsById.contains(id)) {
        ...
        Contributor shared_contributor = m_contributorsById.value(id);
        Foaf foaf = parseCreatorContributorFoafAttributes(property, node_map);
...
EBookMetadata::parseTitleRefines(QString id,
                                 ...
                                 QDomNamedNodeMap node_map)
{
  Title shared_title = m_titlesById.value(id);
  QDomNode node = node_map.namedItem("property");
  if (!node.isNull()) {
    QString property = node.nodeValue().toLower();
    if (property == "title-type") {
      ...
      int old_seq = m_orderedTitles.key(shared_title);
      if (new_seq != old_seq) { // dont change if key is correct.
        if (m_orderedTitles.contains(new_seq)) { // bugger key already exists
          Title existing_title = m_orderedTitles.value(new_seq);
          m_orderedTitles.remove(new_seq);
          m_orderedTitles.remove(old_seq);
          m_orderedTitles.insert(new_seq, shared_title);
                                ...
                                QDomNamedNodeMap& node_map)
{
  ...
  if (id.startsWith("#")) {
    ...
    // is it a creator
    Creator shared_contributor = m_contributorsByName.value(id);
    parseCreatorContributorRefines(
...
bool
EBookMetadata::parseMetadataItem(const QDomNode& metadataNode)
{
...
EBookMetadata::parseTitleMetadata(QDomElement metadata_element)
{
  ...
  if (!node.isNull()) { // alt-rep element is NOT used in EPUB 2.0
    ...
    sharedTitle->alt_rep_list.append(shared_alt_rep);
  }
  node = nodeMap.namedItem("file-as");
  if (!node.isNull()) { // file-as element is NOT used in EPUB 2.0
    FileAs file_as = FileAs(new EBookFileAs());
    file_as->name = node.nodeValue();
    sharedTitle->file_as_list.append(file_as);
  }
  ...
  m_orderedTitles.insert(getNextTitleIndex(), sharedTitle);
}

void
EBookMetadata::parseCreatorMetadata(QDomElement metadataElement)
{
  QDomNamedNodeMap node_map = metadataElement.attributes();
  ...
  if (!node.isNull()) {
    ...
    creator->alt_RepList.append(alt_rep);
  }
...
EBookMetadata::parseContributorMetadata(QDomElement metadata_element)
{
  ...
  if (!node.isNull()) {
    ...
    shared_contributor->alt_RepList.append(alt_rep);
  }

  shared_contributor->name = metadata_element.text();
  if (!shared_contributor->id.isEmpty()) {
    m_contributorsById.insert(shared_contributor->id, shared_contributor);
  }
  m_contributorsByName.insert(shared_contributor->name, shared_contributor);
...
EBookMetadata::parseDescriptionMetadata(QDomElement metadata_element)
{
   ...
   * identical to Creators data. */
  QDomNamedNodeMap node_map = metadata_element.attributes();
  m_description = Description(new EBookDescription());
  QDomNode node = node_map.namedItem("id");
  if (!node.isNull()) { //  EPUB 3.0 only
    m_description->id = node.nodeValue();
  }
  node = node_map.namedItem("dir");
  if (!node.isNull()) { // dir element is NOT used in EPUB 2.0
    m_description->dir = node.nodeValue();
  }
  node = node_map.namedItem("lang");
  if (!node.isNull()) { // lang element is NOT used in EPUB 2.0
  ...
  m_description->text = metadata_element.text();
}
                                      ...
                                      QString key)
{
  ...
  if (shared_creator) {
    ...
    Contributor shared_contributor =
      m_contributorsByName.value(key).dynamicCast<EBookContributor>();
                                       ...
                                       Creator shared_object)
{
  ...
  if (!shared_object.isNull()) {
    ...
    if (!shared_object->name.isEmpty()) {
      ...
      foreach (AltRep alt_rep, shared_object->alt_RepList) {
        writeAltRepAttribute(xml_writer, alt_rep);
      }
...
{
  int highest = 0;
  foreach (int i, m_orderedTitles.keys()) {
    if (i > highest)
      highest = i;
  }
  highest++;
