...
void
ListItem::setDescent(int newDescent)
{
  m_descent = newDescent;
}
...
ListDisplay::paintEvent(QPaintEvent* /*event*/)
{
  ...
  for (auto& item : m_items) {
    ...
    if (!item->icon().isNull()) {
      ...
      painter.drawPixmap(
        rect.left(),
        rect.top() + halfDifference(m_maxItemHeight, item->iconSize().height()),
        pixmap);
    }
    ...
    if (!item->text().isEmpty()) {
      painter.drawText(item->textLeft(),
                       rect.bottom() - fm.descent() -
                         halfDifference(m_maxItemHeight, fm.height()),
                       item->text());
    }
  }
}
...
int
ListDisplay::halfDifference(int large, int small)
{
