...
quint64
EBookAuthorsDB::insertAuthor(PAuthorData author_data)
{
  if (!author_data)
    return 0;
...
EBookAuthorsDB::compareAndDiscard(QStringList names)
{
  ...
  for (auto& value : names) {
    ...
    QList<PAuthorData> list = m_authorBySurname.values(value);
    for (auto& data : list) {
...
EBookAuthorsDB::removeBook(quint64 index)
{
  if (m_authorData.contains(index)) {
    PAuthorData author = m_authorData.value(index);
  ...
  if (file.exists()) {
    ...
    if (authors_map && authors_map.IsMap()) {
      for (YAML::const_iterator it1 = authors_map.begin();
        ...
        PAuthorData author = PAuthorData(new EBookAuthorData());
...
void
EBookAuthorsDB::addAuthor(PAuthorData author_data)
{
