...
PCreator Metadata::creatorFromName(QString name)
{
...
PContributor Metadata::contributorFromName(QString name)
{
...
Metadata::setOrderedTitles(
  const QList<PTitle>& orderedTitles)
{
...
PRights Metadata::rights() const
{
  return m_rights;
}

QDateTime Metadata::date() {
  return m_date;
}

const QMap<UniqueString, PLanguage> &Metadata::languages() const
{
  return m_languages;
}

const QMap<EBookIdentifierScheme::IdentifierScheme, PIdentifier> &Metadata::identifiers() const
{
...
{
  if (tagName == "creator") {
    auto creator = PCreatorContributorBase(
    ...
    if (!node.isNull()) {
      ...
      if (dcterms.isDCTerm()) {
        if (dcterms.term() == DCTerms::DATE) {
          ...
          auto contributor =
            PContributor(new EPubContributor());
          contributor->name = metadataElement.text();
          m_contributorsByName.insert(contributor->name, contributor);
        ...
        if (m_attribution.isNull()) {
          m_attribution =
            PAttribution(new EPubAttribution());
        }
...
{
  //  QDomNamedNodeMap nodeMap = metadataElement.attributes();
  PIdentifier sharedIdentifier =
    PIdentifier(new EBookIdentifier());
  QDomNode node = attributeMap.namedItem("id");
...
{
  //  QDomNamedNodeMap nodeMap = metadataElement.attributes();
  PLanguage sharedLanguage =
    PLanguage(new EPubLanguage());
  QDomNode node = attributeMap.namedItem("id");
  if (!node.isNull()) {
