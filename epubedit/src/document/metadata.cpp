...
PCreator
Metadata::creatorFromName(QString name)
{
  return m_creatorsByName.value(name);
}

QStringList
Metadata::contributorList()
{
  QStringList names;
...
PContributor
Metadata::contributorFromName(QString name)
{
...
void
Metadata::setOrderedTitles(const QList<PTitle>& orderedTitles)
{
...
void
Metadata::setCalibre(const PCalibre& calibre)
{
  m_calibre = calibre;
}

PRights
Metadata::rights() const
{
  return m_rights;
}
...
const QMap<UniqueString, PLanguage>&
Metadata::languages() const
{
  return m_languages;
}

const QMap<EBookIdentifierScheme::IdentifierScheme, PIdentifier>&
Metadata::identifiers() const
{
  return m_identifiers;
}

NavListItem*
Metadata::navList() const
{
  return m_navList;
}

void
Metadata::setNavList(NavListItem* newNavList)
{
  m_navList = newNavList;
}

void
Metadata::writeCreator(QXmlStreamWriter* xml_writer,
...
void
Metadata::writeContributor(QXmlStreamWriter* xml_writer,
                           PCreatorContributorBase shared_creator)
{
                                ...
                                QDomNamedNodeMap& attributeMap)
{
  ...
  auto sharedSource = PSource(new EPubSource());
...
Metadata::parseDCPublisherMetadata(const QDomElement& metadataElement,
                                   QDomNamedNodeMap& attributeMap)
{
  auto contributor = PContributor(new EPubContributor());
  auto node = attributeMap.namedItem("id");
  if (!node.isNull()) {
...
{
  auto node = attributeMap.namedItem("id");
  auto sharedFormat = PFormat(new EPubFormat());
  if (!node.isNull()) { // id element is NOT used in EPUB 2.0
...
{
  auto node = attributeMap.namedItem("id");
  auto sharedType = PType(new EPubType());
  if (!node.isNull()) { // id element is NOT used in EPUB 2.0
...
Metadata::parseDCRelationMetadata(const QDomElement& metadataElement,
                                  QDomNamedNodeMap& attributeMap)
{
  auto sharedRelation = PRelation(new EPubRelation());
  auto node = attributeMap.namedItem("id");
  if (!node.isNull()) { // id element is NOT used in EPUB 2.0
...
Metadata::parseDCCoverageMetadata(const QDomElement& metadataElement,
                                  QDomNamedNodeMap& attributeMap)
{
  auto sharedCoverage = PCoverage(new EPubCoverage());
  auto node = attributeMap.namedItem("id");
  if (!node.isNull()) { // id element is NOT used in EPUB 2.0
...
Metadata::parseDCRightsMetadata(const QDomElement& metadataElement,
                                QDomNamedNodeMap& attributeMap)
{
  auto sharedRights = PRights(new EPubRights());
  auto node = attributeMap.namedItem("id");
  if (!node.isNull()) { // id element is NOT used in EPUB 2.0
...
{
  if (tagName == "creator") {
    auto creator = PCreatorContributorBase(new EPubCreatorContributorBase());
    auto node = attributeMap.namedItem("file-as"); // 2.0 & 3.0
    if (!node.isNull()) {
      auto file_as = PFileAs(new EPubFileAs());
...
void
Metadata::parseTitleDateRefines(PTitle sharedTitle,
                                QDomElement& metadataElement)
{
...
void
Metadata::parseCreatorContributorRefines(PCreatorContributorBase creator,
                                         QDomElement& metadataElement,
                                         ...
                                         QDomNamedNodeMap attributeMap)
{
  ...
  if (!node.isNull()) {
    ...
    if (property == "role") {
      ...
      auto altRep = PAltRep(new EPubAltRep());
...
Metadata::parseTitleRefines(const QString& id,
                            ...
                            QDomNamedNodeMap attributeMap)
{
  PTitle sharedTitle = m_titlesById.value(id);
  ...
  if (!node.isNull()) {
    ...
    if (property == "title-type") {
      ...
      PAltRep alt_rep = PAltRep(new EPubAltRep());
      ...
      PFileAs file_as = PFileAs(new EPubFileAs());
      file_as->name = node.nodeValue();
      node = attributeMap.namedItem("lang");
      if (!node.isNull()) {
        file_as->lang = node.nodeValue();
      ...
      if (dcterms.isDCTerm()) {
        if (dcterms.term() == DCTerms::DATE) {
          ...
          if (!m_source.isNull()) {
            ...
            auto shared_source = PSource(new EPubSource());
          ...
          if (!m_rights.isNull()) {
            ...
            auto shared_rights = PRights(new EPubRights());
          ...
          if (!keys.contains(creator, Qt::CaseInsensitive)) {
            ...
            // by the user.
            auto shared_creator = PCreator(new EPubCreator());
          ...
          if (keys.contains(title, Qt::CaseInsensitive)) {
            auto shared_title = PTitle(new EPubTitle());
          ...
          if (!keys.contains(contributor, Qt::CaseInsensitive)) {
            ...
            // is different. I think that that will have to be edited manually
            // by the user.
            auto shared_contributor = PContributor(new EPubContributor());
        ...
        if (m_attribution.isNull()) {
          m_attribution = PAttribution(new EPubAttribution());
        }
...
Metadata::parseDCTitleMetadata(QDomElement metadataElement,
                               QDomNamedNodeMap& attributeMap)
{
  PTitle sharedTitle = PTitle(new EPubTitle());
  ...
  node = attributeMap.namedItem("alt-rep");
  if (!node.isNull()) { // alt-rep element is NOT used in EPUB 2.0
    PAltRep sharedAltRep = PAltRep(new EPubAltRep());
  ...
  node = attributeMap.namedItem("file-as");
  if (!node.isNull()) { // file-as element is NOT used in EPUB 2.0
    PFileAs fileAs = PFileAs(new EPubFileAs());
                                 ...
                                 QDomNamedNodeMap& attributeMap)
{
  ...
  auto creator = PCreator(new EPubCreator());
  auto node = attributeMap.namedItem("id");
                                     ...
                                     QDomNamedNodeMap& nodeMap)
{
  ...
  if (!node.isNull()) {
    auto fileAs = PFileAs(new EPubFileAs());
    fileAs->name = node.nodeValue();
                                     ...
                                     QDomNamedNodeMap& attributeMap)
{
   ...
   * identical to Creators data. */
  //  QDomNamedNodeMap nodeMap = metadataElement.attributes();
  m_description = PDescription(new EPubDescription());
  QDomNode node = attributeMap.namedItem("id");
  if (!node.isNull()) { //  EPUB 3.0 only
...
{
  //  QDomNamedNodeMap nodeMap = metadataElement.attributes();
  PIdentifier sharedIdentifier = PIdentifier(new EBookIdentifier());
  QDomNode node = attributeMap.namedItem("id");
...
{
  //  QDomNamedNodeMap nodeMap = metadataElement.attributes();
  PLanguage sharedLanguage = PLanguage(new EPubLanguage());
  QDomNode node = attributeMap.namedItem("id");
  if (!node.isNull()) {
                                 ...
                                 QDomNamedNodeMap& nodeMap)
{
  //  auto nodeMap = metadataElement.attributes();
  auto node = nodeMap.namedItem("id");
  auto sharedSubject = PSubject(new EPubSubject());
                                 ...
                                 QString key)
{
  ...
  PCreatorContributorBase shared_object;
  QString name;
  ...
  if (shared_creator) {
    ...
    PContributor shared_contributor =
                                ...
                                bool first)
{
   ...
   */
  PLanguage sharedLanguage = m_languages.value(key);
...
{
  PSubject shared_subject = m_subjects.value(key);
...
{
  PIdentifier shared_identifier = m_identifiers.value(key);
...
void
Metadata::writeTitle(QXmlStreamWriter* xml_writer, PTitle sharedTitle)
{
  ...
  if (!sharedTitle->title.isEmpty()) {
    ...
    QList<PAltRep> alt_reps = sharedTitle->altRepList;
    for (PAltRep& alt_rep : alt_reps) {
      writeAltRepAttribute(xml_writer, alt_rep);
    }
    QList<PFileAs> file_as_list = sharedTitle->fileAsList;
    for (PFileAs& file_as : file_as_list) {
...
{
  PTitle shared_title = m_orderedTitles.value(key);
  QString title;
...
{
  PSource shared_source = m_source;
...
{
  PFormat shared_format = m_format;
...
{
  PRelation shared_publisher = m_relation;
  if (!shared_publisher->name.isEmpty()) {
...
{
  PRights shared_rights = m_rights;
...
{
  PCoverage shared_coverage = m_coverage;
...
void
Metadata::writeAltRepAttribute(QXmlStreamWriter* xml_writer, PAltRep alt_rep)
{
  if (alt_rep) {
...
void
Metadata::writeFileAsAttribute(QXmlStreamWriter* xml_writer, PFileAs file_as)
{
  if (file_as) { // 3.0
...
void
Metadata::writeRoleAttribute(QXmlStreamWriter* xml_writer, PMarcRelator relator)
{
  if (relator->type() != MarcRelator::NO_TYPE) {
 ...
 */
QString
Metadata::writeCreatorContributor(QString tag_name,
                                  QXmlStreamWriter* xml_writer,
                                  PCreatorContributorBase sharedObject)
{
  QString name;
...
EPubCreatorContributorBase::EPubCreatorContributorBase()
  : fileAs(PFileAs(new EPubFileAs()))
  , altRep(PAltRep(new EPubAltRep()))
  , relator(PMarcRelator(new MarcRelator()))
{}
